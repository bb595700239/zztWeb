	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if(opts.isJump){
				...
				panel.append(jQuery("<span><input type='text' style='text-align:center;font-weight:bold' id='jump-index' title='请输入正整数' size='1' /></span>"));
				...
				var jump = jQuery("<span class='jump-page'>"+opts.jump_text+"</span>").bind("click",function(evt){
					...
					index = jumpinput.val();
					if(index == null || index == ""){
						$.alert('<i class="i-icon3 i-round-warn"></i>'+opts.jump_null_text,true,null,5000,null,true);
						return;
					}
					if(/^\d+$/.test(index)){
						if(index > numPages() || index < 1){
							$.alert('<i class="i-icon3 i-round-warn"></i>'+opts.jump_outofrange_text,true,null,5000,null,true);
							jumpinput.val("");
							return;
						}
						index-=1;
						...
						$.alert('<i class="i-icon3 i-round-warn"></i>'+opts.jump_format_text,true,null,5000,null,true);
						jumpinput.val("");
						return;
					}
